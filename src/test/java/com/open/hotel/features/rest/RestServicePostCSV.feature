...
Feature: Create the user in a gorest api site
  ...
  #  When I validate "krishna73" from "name" node in JSON response - JSON path "data.name"
    When I validate "Male" from "gender" node in JSON response - JSON path "data.gender"
    When I validate "krishna73@gmail.com" from "email" node in JSON response - JSON path "data.email"
